...
//                   className="w-full h-[calc(100%+10px)] sm:h-[calc(100%+10px)] md:h-[calc(100%+20px)] lg:h-[calc(50%+20px)]" // Adjust card height for mobile and larger screens
//                 />
//               </div>
//             ))}
//           </div>
...
export default function PropertySearchList({ currentPage, resultsPerPage }) {
  const [properties, setProperties] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
          ...
          {paginatedProperties.map((property) => (
            ...
            >
              <SearchResultCard
                ...
                images={property.images}
                onClick={() => handleCardClick(property)}
                className="w-full h-[calc(100%+10px)] sm:h-[calc(100%+10px)] md:h-[calc(100%+20px)] lg:h-[calc(50%+20px)]"
              />
            </div>
          ))}
        </div>
      ) : (
        <div className="text-gray-700 flex text-center justify-center">
          No properties found matching your criteria.
        </div>
      )}
    </div>
